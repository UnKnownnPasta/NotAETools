...
module.exports = {
    ...
    async execute (client, i) {
        ...
        const frame = i.options.getString('frame', true)
        const mstype = i.options.getString('missiontype', true)
        const dura = i.options.getInteger('duration', false) ?? 0
        ...
        possFrames.forEach(fm => {
            ...
            if (fm.data.custom_id !== 'fhost-❌') { farmEmbed.addFields({ name: `${fm.data.label}`, value: `${fm.data.custom_id == `fhost-${frame}` ? `<@${i.user.id}>` : 'None'}`, inline: true }) }
        })
