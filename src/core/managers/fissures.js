...
class FissureManager {
    constructor() {
        this.client = null;
        this.latestFissures = []
        this.interval = 600_000
        this.channel = departments.treasury.fissureChannel
    }

    setClient(client) { this.client = client; }

    async refreshGuilds() { await this.client.guilds.fetch({ force: true }) }
    ...
    async syncFissures() {
        if (!this.client) return logger.error(`No client initialized : Fissures`)
        try {
            ...
            this.latestFissures = [normalEmbed, spEmbed];
            ...
            logger.error('Fissure Manager Error', error);
        }
    }
}

module.exports = new FissureManager();
